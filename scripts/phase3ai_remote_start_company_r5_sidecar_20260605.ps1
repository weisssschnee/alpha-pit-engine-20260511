...
$RepoRoot = "D:\HermesWorker\workspace\phase3ai_overnight_sidecar_current"
...
$RunRoot = "D:\p3ai\overnight_company_20260605_r5_sidecar"
$StatusPath = Join-Path $RunRoot "sidecar_status.jsonl"
...
function Invoke-Leg($name, $mode, $canaryShards, $mainShards, $candidates, $windows, $beamWidth, $maxBeam, $familyShare, $exploration, $previousLimit, $rewardLimit, $halvingFraction, $halvingMin, $minEval) {
  foreach ($stage in @("canary","main")) {
    ...
    $maxActive = if ($isCanary) { 1 } else { 1 }
    ...
    $argsList = @(
      ...
      "--job-id", "phase3ai_company_r5_sidecar_$($name)_$stage",
      "--machine", "company",
...
Write-Status ([ordered]@{time=(Get-Date).ToString("s"); event="sidecar_begin"; run_root=$RunRoot; note="adds one extra memory-safe company worker lane without changing r5 primary"})
Invoke-Leg "sc1_rx_deep_novelty_ms" "rx_typed_beam" 4 160 28 120 1024 196608 0.035 0.88 28 14 0.45 28 28
Invoke-Leg "sc2_forward_microstructure_ms" "forward_first" 4 128 56 96 64 8192 0.08 0.86 28 14 0.35 40 40
Write-Status ([ordered]@{time=(Get-Date).ToString("s"); event="sidecar_end"; run_root=$RunRoot})
