...
$RunRoot = "G:\Project_V7_Rotation\runtime\phase3ai_overnight_local_20260605"
...
function Invoke-SearchLeg($leg, [bool]$canary) {
  ...
  $argsList = @(
    ...
    "--memory-base", "G:\Project_V7_Rotation\runtime\phase3ai_overnight_local_20260605",
    "--shard-count", "$shards",
...
foreach ($leg in $legs) {
  if ((Get-Date) -gt $deadline) { Write-Status ([ordered]@{time=(Get-Date).ToString("s"); event="deadline_stop_before_leg"; leg=$leg.name}); break }
  ...
  if ((Get-Date) -gt $deadline) { Write-Status ([ordered]@{time=(Get-Date).ToString("s"); event="deadline_stop_after_canary"; leg=$leg.name}); break }
