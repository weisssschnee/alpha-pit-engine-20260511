...
$RepoRoot = "D:\HermesWorker\workspace\phase3ai_overnight_sidecar2_current"
...
$RunRoot = "D:\p3ai\overnight_company_20260605_r5_sidecar2"
$StatusPath = Join-Path $RunRoot "sidecar2_status.jsonl"
...
function Invoke-Leg($name, $mode, $canaryShards, $mainShards, $candidates, $windows, $beamWidth, $maxBeam, $familyShare, $exploration, $previousLimit, $rewardLimit, $halvingFraction, $halvingMin, $minEval) {
  foreach ($stage in @("canary","main")) {
    ...
    $argsList = @(
      ...
      "--job-id", "phase3ai_company_r5_sidecar2_$($name)_$stage",
      "--machine", "company",
...
Write-Status ([ordered]@{time=(Get-Date).ToString("s"); event="sidecar2_begin"; run_root=$RunRoot; note="adds fourth memory-safe company worker lane"})
Invoke-Leg "sc3_forward_event_microstructure_ms" "forward_first" 4 128 56 96 64 8192 0.08 0.88 28 14 0.35 40 40
Invoke-Leg "sc4_rx_event_orthogonal_ms" "rx_typed_beam" 4 96 28 120 1024 196608 0.035 0.88 32 16 0.45 28 28
Write-Status ([ordered]@{time=(Get-Date).ToString("s"); event="sidecar2_end"; run_root=$RunRoot})
